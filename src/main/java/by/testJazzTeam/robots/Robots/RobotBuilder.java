...
public class RobotBuilder implements RobotInterface {

    private WebSocketEventSender webSocketEventSender;
    private Integer idRobot;
    private final String typeJob = "build";
    ...
    public RobotBuilder(Integer idRobot, Task jobQueue, WebSocketEventSender webSocketEventSender) {
    ...
    public void job() {
        ...
        if (jobQueue.size() == 0) {
            robotWorkLog.setContent("idRobot Builder : " + idRobot + " status is free ");
            ...
            for (int i = 0; i <= jobQueue.size(); i++) {
                ...
                for (int j = 0; j <= taskTemp.getNumberOfStepsInTheJob(); j++) {
                    robotWorkLog.setContent("idRobot Builder : " + idRobot + " Task : " + taskTemp.getTypeTask() + " № Task " + j);
                    webSocketEventSender.sendLogToWebUI(robotWorkLog);
                    try {
